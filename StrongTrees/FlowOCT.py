...
class FlowOCT:
    def __init__(self, X, y, tree, _lambda, time_limit, mode):
        ...
        self.mode = mode
        self.X = X
        self.X_col_idx = np.arange(0, self.X.shape[1])
        self.y = y
        ...
        '''
        # cat_features is the column indices of the X matrix
    ...
    def create_primal_problem(self):
        ...
        self.b = self.model.addVars(
            self.tree.Nodes, self.X_col_idx, vtype=GRB.BINARY, name='b')
        ...
        for i in self.datapoints:
            self.model.addConstrs((self.z[i, int(self.tree.get_left_children(n))] <= self.m[i] * quicksum(
                self.b[n, f] for f in self.X_col_idx if self.X[i, f] == 0)) for n in self.tree.Nodes)

        # z[i,r(n)] <= m[i] * sum(b[n,f], f if x[i,f]=1)
        # forall i, n in Nodes
        for i in self.datapoints:
            self.model.addConstrs((self.z[i, int(self.tree.get_right_children(n))] <= self.m[i] * quicksum(
                self.b[n, f] for f in self.X_col_idx if self.X[i, f] == 1)) for n in self.tree.Nodes)

        # sum(b[n,f], f) + p[n] + sum(p[m], m in A(n)) = 1   forall n in Nodes
        self.model.addConstrs(
            (quicksum(self.b[n, f] for f in self.X_col_idx) +
             self.p[n] + quicksum(
        ...
        if self.mode == "classification":
            ...
            for n in self.tree.Nodes + self.tree.Leaves:
                self.model.addConstrs(
                    ...
                    self.y[i] * self.p[n] + self.beta[n, 1]
                    for i in self.datapoints)
