...
`RobustOCT` is an MIO-based method for building optimal classification trees robust to these distribution shifts for data with integer-valued features. Details on the method can be found in the paper (Justin et al. 2021, 2023).
...
To fit a `RobustOCT`, the expected distribution shift must be specified. This is through the `costs` and `budget` parameters of the `RobustOCT.fit()` function. `RobustOCT` contains a function `probabilities_to_costs` to help generate the values for `costs` and `budget` based on knowledge from the application.
