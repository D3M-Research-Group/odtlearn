...
class FlowOPTSingleSink(OptimalPrescriptiveTree):
    ...
    derived classes that implement a specific prescriptive tree method, such as `FlowOPT_IPW`.

    This class extends the `OptimalPrescriptiveTree` class to learn optimal prescriptive trees
    ...
    formulation with a single sink node. The `_define_variables` method defines the decision
    variables, including the tree structure variables (`_b`, `_p`, `_w`) and the flow variable (`_z`).
    ...
    The class inherits the `fit`, `predict`, `print_tree`, and `plot_tree` methods from the
    `OptimalPrescriptiveTree` class to learn the optimal prescriptive tree, make predictions,
