...
def get_cut_expression(master, b, w, path, xi, v, i):
    expr = LinExpr(0)
    node_leaf_cutoff = np.power(2, master.tree.depth) # anything at or above this number is a leaf
    ...
    for x in range(len(path)):
        ...
        if n < node_leaf_cutoff:
            if x == len(path)-1:
                ...
                expr += quicksum(master.b[n, f, theta] for (f,theta) in master.f_theta_indices)
...
def get_all_terminal_paths(master, b, w, terminal_nodes=[], path_dict={}, feature_path_dict={}, assignment_dict={},
    cutoff_dict = {}, curr_node=1, curr_path=[1], curr_feature_path = [], curr_cutoff_path = []):
    ...
    for k in master.labels:
        if w[curr_node, k] > 0.5: # w[n,k] == 1
            ...
            return new_terminal_nodes, new_path_dict, new_feature_path_dict, new_assignment_dict, new_cutoff_dict
    ...
    left_terminal, left_paths, left_feature, left_assign, left_cutoff = get_all_terminal_paths(master, b, w, 
        terminal_nodes=terminal_nodes, path_dict=path_dict, feature_path_dict=feature_path_dict, assignment_dict=assignment_dict,
        cutoff_dict=cutoff_dict, curr_node=left_node, curr_path=left_path, curr_feature_path=curr_feature_path + [curr_feature],
    ...
    right_terminal, right_paths, right_feature, right_assign, right_cutoff = get_all_terminal_paths(master, b, w, 
        terminal_nodes=terminal_nodes, path_dict=path_dict, feature_path_dict=feature_path_dict, assignment_dict=assignment_dict,
        cutoff_dict=cutoff_dict, curr_node=right_node, curr_path=right_path, curr_feature_path=curr_feature_path+[curr_feature],
        curr_cutoff_path=curr_cutoff_path + [curr_theta])
...
def shortest_path_solver(master, i, label, terminal_nodes, terminal_path_dict, 
    terminal_features_dict, terminal_assignments_dict, terminal_cutoffs_dict, initial_xi,
    initial_mins, initial_maxes):
    ...
    for j in terminal_nodes:
        ...
        curr_cost = master.eta * int(curr_v) # Start with cost if correctly classify point
        best_so_far = True
...
def subproblem(master, i, terminal_nodes, terminal_path_dict, terminal_features_dict, 
    terminal_assignments_dict, terminal_cutoffs_dict, initial_xi={}, initial_mins={}, initial_maxes={}):
    label_i = master.y[i]
    target = [] # list of nodes whose edge to the sink is part of the min cut

    # Solve shortest path problem via DFS w/ pruning
    target, cost, xi, v = shortest_path_solver(master, i, label_i, terminal_nodes, terminal_path_dict, terminal_features_dict, 
        terminal_assignments_dict, terminal_cutoffs_dict, initial_xi, initial_mins, initial_maxes)
...
def mycallback(model, where):
    ...
    if where == GRB.Callback.MIPSOL:
        ...
        xi_dict = {} # Stores set of xi's from shortest path problem (feature perturbations)
        ...
        for i in model._master.datapoints:
            ...
            if k != model._master.y[i]:
                ...
                curr_expr = get_cut_expression(model._master, b, w, nom_path, initial_xi, False, i)
        ...
        # Get all paths
        terminal_nodes, terminal_path_dict, terminal_features_dict, terminal_assignments_dict, terminal_cutoffs_dict = \
            get_all_terminal_paths(model._master, b, w)                 
        for i in correct_points:
            path, xi, cost, v = subproblem(model._master, i, terminal_nodes, 
                terminal_path_dict, terminal_features_dict, terminal_assignments_dict, terminal_cutoffs_dict,
                initial_xi=copy.deepcopy(initial_xi), initial_mins = copy.deepcopy(initial_mins),
        ...
        while True:
            ...
            i = current_point[1]
            whole_expr.add(get_cut_expression(model._master, b, w, path_dict[i], xi_dict[i], v_dict[i], i))
            total_cost += curr_cost
        
        added_cut = (round(sum(t)) > len(priority_queue)) # current sum of t is larger than RHS -> violated constraint(s)
        if added_cut:
            while len(priority_queue) != 0:
                current_point = heapq.heappop(priority_queue)
                i = current_point[1]
                whole_expr.add(get_cut_expression(model._master, b, w, nom_path_dict[i], initial_xi, False, i))
            model.cbLazy(quicksum(model._master.t[i] for i in model._master.datapoints) <= whole_expr)
            model._total_cuts += 1
